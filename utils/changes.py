...
async def send_changes_to_users(bot: Bot, date: str):
    ...
    for user in users_with_setting:
        ...
        text = (
            ...
            if check_if_group_in_changes(group["name"], date)
            else (
                f"🔔 Появились изменения на <b>{date}</b>.\n"
    ...
    with pdfplumber.open(f"./data/changes/changes_{date}.pdf") as pdf:
        for page in pdf.pages:
            ...
            for line in text:
                if (
                    group_name in line
                    and ("прием" not in line)
                    and ("подготовка" not in line)
                    and ("пересдача" not in line)
                    and ("консультация" not in line)
                    and ("профориентационное" not in line)
                ):
                    return True
    return False
