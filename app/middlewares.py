    ...
    async def __call__(self, message: Message, state: FSMContext) -> bool:
        ...
        if user is not None and user.group is None:
            stmt = (
                ...
                and message.text.strip() != "/repair"
                and (await state.get_state() != "setting_group:group_name")
            )
            if stmt:
                await message.answer(
                    "➡️ Для продолжения использования бота присоединитесь к группе\n\nИз <b>Пдо-16</b>?\n👉 https://t.me/homew0rk_bot?start=invite_svmeP8pb_pdo-16",
                    parse_mode="html",
                )
                return False
        return True
...
class MsgLoggerMiddleware(BaseException):
    async def __call__(
        ...
        user_name = (
            f"{event.from_user.first_name or ''}{' ' + event.from_user.last_name if event.from_user.last_name else ''}"
        )
