    ...
    if user.role != 0:
        ...
        if len(args) > 1 and args[0] == "/start":
                    ...
                    if group.uid == user.group.uid:
                        ...
                        if user.is_leader:
                            await message.answer(
                                "Вы сможете присоединиться к другой группе как только передадите права лидерства другому человеку."
                            )
                        else:
                            if check_time_moved(user):
                                await message.answer(
                                    f"Вы хотите присоединиться к другой группе (<b>{group.name}</b>)?\n<i>В случае присоединения, вы не сможете сменить группу в следующие 48 часов.</i>",
                                    parse_mode="html",
                                    reply_markup=kb.do_join_to_group_keyboard,
                                )
                                await state.set_state(joiningToGroup.group_name)
                                await state.update_data(group_name=group.name)
                            else:
                                await message.answer(
                                    f"Вы временно не можете изменять группу\n<i>Ограничение на 48 часов</i>",
                                    parse_mode="html",
                                    reply_markup=await kb.get_start_keyboard(user),
                                )
                else:
                    await message.answer(
                        f"Вы хотите присоединиться к группе <b>{group.name}</b>?\n<i>В случае присоединения, вы не сможете сменить группу в следующие 48 часов.</i>",
                        parse_mode="html",
                        reply_markup=kb.do_join_to_group_keyboard,
                    )
                    await state.set_state(joiningToGroup.group_name)
            ...
            if user.group is None:
                ...
                await message.answer(
                    "Тут можно посмотреть домашнее задание. Выбери опцию.",
                    reply_markup=await kb.get_start_keyboard(user),
                )
    ...
    if message.text.strip().lower() in all_groups_names:
        ...
        if group:
            if group.is_equipped:
                ...
                await message.answer(
                    "Эта группа еще не зарегистрирована в системе, при подтверждении вы станете <b>лидером</b> группы \n\n(о том что может лидер группы вы можете узнать в /about)",
                    parse_mode="html",
                    reply_markup=kb.create_group_keyboard,
                )
    else:
