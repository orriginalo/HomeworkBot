...
async def adding_admin_handler(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "Перешлите сообщение или введите id пользователя для добавления:", reply_markup=kb.back_keyboard
    )
...
async def add_admin_id(message: Message, state: FSMContext):
    ...
    if user_role == None:
        ...
        await message.answer(
            f"✅ Пользователь добавлен в администраторы.",
            parse_mode="html",
            reply_markup=await kb.get_start_keyboard(user),
        )
    else:
...
async def remove_admin_id(message: Message, state: FSMContext):
    ...
    if user_to_remove.role == None:
        await message.answer(
            f"❌ Пользователь не существует или скрыл id.", reply_markup=await kb.get_start_keyboard(user)
        )
        await state.clear()
        return
        ...
        await message.answer(
            f"✅ Пользователь удален из администраторов.", reply_markup=await kb.get_start_keyboard(user)
        )
    else:
        await message.answer(
            f"❌ Пользователь не является администратором.", reply_markup=await kb.get_start_keyboard(user)
        )
    ...
    if adding_user:
        if user.role <= 3:
            if adding_user.group.uid != user.group.uid:
                await message.answer(
                    "❌ Вы не можете сделать этого пользователя добавлятелем, поскольку он не входит в вашу группу.",
                    reply_markup=await kb.get_start_keyboard(user),
                )
                await state.clear()
                return
        ...
        if adding_user.role == None:
            ...
            await message.answer(
                f"✅ Пользователь добавлен в добавлятелей.", reply_markup=await kb.get_start_keyboard(user)
            )
        else:
            await message.answer(
                f"❌ Пользователь уже является добавлятелем.", reply_markup=await kb.get_start_keyboard(user)
            )
    else:
        await message.answer(
            f"❌ Пользователь не существует, или его нет в базе (пусть напишет боту).",
            reply_markup=await kb.get_start_keyboard(user),
        )
...
async def remove_adder_handler(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "Перешлите сообщение или введите id добавлятеля для удаления:", reply_markup=kb.back_keyboard
    )
    ...
    if adding_user:
        if user["role"] <= 3:
            if adding_user["group_id"] != user["group_id"]:
                await message.answer(
                    "❌ Вы не можете удалить роль добавлятеля у этого пользователя, поскольку он не входит в вашу группу."
                )
                await state.clear()
                return
        ...
        if adding_user["role"] == None:
            ...
            await message.answer(
                f"✅ Пользователь удален из добавлятелей.",
                parse_mode="html",
                reply_markup=await kb.get_start_keyboard(user),
            )
            ...
            await message.answer(
                f"❌ Пользователь не является добавлятелем.",
                parse_mode="html",
                reply_markup=await kb.get_start_keyboard(user),
            )
...
async def get_db_backup_handler(call: CallbackQuery):
    ...
    await call.message.answer_document(
        dbfile, caption=f"Резервная копия базы данных <b>{backup_date_time}</b>", parse_mode="html"
    )
...
async def get_logs_backup_handler(call: CallbackQuery):
    ...
    await call.message.answer_document(
        logfile, caption=f"Резервная копия логов <b>{backup_date_time}</b>", parse_mode="html"
    )
    ...
    if user["role"] >= 4:
        ...
        for user in users:
            if user.tg_id != 1579774985:
                try:
                    await message.answer(
                        f"✉️ <a href='tg://user?id={user['tg_id']}'>{user['tg_id']}</a>...", parse_mode="html"
                    )
                    ...
                    await message.answer(
                        f"Не удалось отправить сообщение пользователю <a href='tg://user?id={user['tg_id']}'>{user['tg_id']}</a>"
                    )
        await message.answer("✅ Сообщение отправлено всем пользователям.")
        await message.answer("Выберите опцию:", reply_markup=await kb.get_start_keyboard(user))
