...
async def add_homework(subject: str, task: str, group_id: int, added_by: int, from_date_ts: int, to_date_ts: int = None, **kwargs):
  try:
    async with session() as s:
      ...
      return HomeworkSchema.model_validate(homework, from_attributes=True)
...
async def get_homeworks(*filters):
  async with session() as s:
    ...
    homeworks = [HomeworkSchema.model_validate(homework, from_attributes=True) for homework in homeworks]
    return homeworks
...
async def get_homework_by_id(homework_id: int):
  try:
    async with session() as s:
      ...
      result = await s.execute(stmt)
      homework = result.scalar_one_or_none()
      return HomeworkSchema.model_validate(homework, from_attributes=True) if homework else None
  except Exception as e:
