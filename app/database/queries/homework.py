...
async def add_homework(
    subject: str, task: str, group_id: int, added_by: int, from_date_ts: int, to_date_ts: int = None, **kwargs
):
    try:
...
async def del_homework(homework_id: int):
    try:
        async with session() as s:
            stmt = (
                select(Homework)
                .where(Homework.uid == homework_id)
...
async def get_homeworks_by_subject(
    subject: str, limit_last: Optional[bool] = False, limit_last_count: int = 2, group_id: int = None
) -> List[HomeworkRelSchema] | None:
    try:
        async with session() as s:
            stmt = (
                select(Homework)
                .where(Homework.subject == subject)
                .options(selectinload(Homework.user), selectinload(Homework.group))
            )
    ...
    try:
        async with session() as s:
            ...
            if homework:
                ...
                stmt = (
                    select(Schedule)
                    .where(Schedule.subject == homework.subject, Schedule.timestamp > homework.from_date)
